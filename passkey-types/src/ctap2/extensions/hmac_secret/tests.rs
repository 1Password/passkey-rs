...
const GOOD_SALT1: [u8; 32] = [
    130, 250, 15, 242, 237, 2, 78, 230, 76, 63, 184, 229, 40, 172, 4, 60, 75, 182, 244, 15, 109,
    248, 177, 205, 235, 65, 32, 16, 183, 12, 145, 39,
];
const GOOD_SALT2: [u8; 32] = [
    188, 232, 220, 195, 110, 115, 163, 139, 67, 124, 35, 10, 117, 252, 33, 207, 48, 16, 59, 32, 69,
    95, 121, 238, 217, 110, 160, 25, 20, 97, 164, 140,
];
const GOOD_SALT1_AND_2: [u8; 64] = [
    130, 250, 15, 242, 237, 2, 78, 230, 76, 63, 184, 229, 40, 172, 4, 60, 75, 182, 244, 15, 109,
    248, 177, 205, 235, 65, 32, 16, 183, 12, 145, 39, 188, 232, 220, 195, 110, 115, 163, 139, 67,
    124, 35, 10, 117, 252, 33, 207, 48, 16, 59, 32, 69, 95, 121, 238, 217, 110, 160, 25, 20, 97,
    164, 140,
];
#[test]
...
fn from_64_byte_slice() {
    ...
    let salt = HmacSecretSaltOrOutput::try_new(GOOD_SALT1.as_slice(), Some(GOOD_SALT2.as_slice()))
        .expect("Failed to parse slice of both salts");
    assert_eq!(&GOOD_SALT1, salt.first());
    assert!(salt.has_salt2);
    assert_eq!(salt.second(), Some(GOOD_SALT2.as_slice()));
...
fn from_incorrectly_sized_byte_slice() {
    let too_short = AvailableRng::random_vec(31);
    let between = AvailableRng::random_vec(33);
    let too_long = AvailableRng::random_vec(65);
    ...
    let correct = AvailableRng::random_vec(32);
...
fn from_correct_cbor() {
    ...
    let remote_one_salt = cbor!({
        ...
        0x03 => Value::Bytes(AvailableRng::random_vec(32))
    })
    .unwrap();
...
fn cbor_round_trip_one_salt() {
    ...
    let one_salt = HmacGetSecretInput {
        ...
        salt_auth: AvailableRng::random_vec(32).into(),
    ...
    let expect_one_salt: HmacGetSecretInput = ciborium::de::from_reader(buf.as_slice()).unwrap();
