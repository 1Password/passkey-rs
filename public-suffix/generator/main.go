...
// This program generates tld_list.rs and tld_list_test.rs based on the authoritative
// public suffix list at https://publicsuffix.org/list/public_suffix_list.dat.
...
var (
	...
	comments       = flag.Bool("comments", false, "generate tld_list.rs comments, for debugging")
	subset         = flag.Bool("subset", false, "generate only a subset of the full table, for debugging")
	v              = flag.Bool("v", false, "verbose output (to stderr)")
	strct          = flag.String("struct", "TLDList", "name of the struct to be generated")
	crate          = flag.Bool("crate", true, "If present, generates an impl for public_suffix::Table. If absent, generates crate::Table.")
	fileOutputPath = flag.String("output-path", "./src", "The path to the directory where outputs should be placed. Defaults to ./src")
	fileBaseName   = flag.String("base-name", "tld_list", "The base name of the file. Defaults to tld_list.rs and tld_list_tests.rs")
)
...
func main1() error {
	...
	filePath := fmt.Sprintf("%s%s.rs", *fileOutputPath, *fileBaseName)
	if err := generate(printReal, &root, filePath); err != nil {
		return err
	}
	testFilePath := fmt.Sprintf("%s%s_test.rs", *fileOutputPath, *fileBaseName)
	if err := generate(printTest, &root, testFilePath); err != nil {
		return err
	}
	return nil
}
...
func printTest(w io.Writer, n *node) error {
	...
	// The Go version generates a `TestSlowPublicSuffix` here; we
	// skipped that.

	fmt.Fprintf(w, "pub const NODE_LABELS: &[&str] = &[\n")
...
impl Table for %s {
    const NODES_BITS_CHILDREN: u32 = %d;
    const NODES_BITS_ICANN: u32 = %d;
    const NODES_BITS_TEXT_OFFSET: u32 = %d;
    const NODES_BITS_TEXT_LENGTH: u32 = %d;

    const CHILDREN_BITS_WILDCARD: u32 = %d;
    const CHILDREN_BITS_NODE_TYPE: u32 = %d;
    const CHILDREN_BITS_HI: u32 = %d;
    const CHILDREN_BITS_LO: u32 = %d;

    const NODE_TYPE_NORMAL: u32 = %d;
    const NODE_TYPE_EXCEPTION: u32 = %d;
...
`
	fmt.Fprintf(w, header,
		crate_name, *strct, *strct, nodesBitsChildren, nodesBitsICANN, nodesBitsTextOffset, nodesBitsTextLength,
		childrenBitsWildcard, childrenBitsNodeType, childrenBitsHi, childrenBitsLo,
		nodeTypeNormal, nodeTypeException, len(n.children))
...
`,
	...
	for i, c := range childrenEncoding {
		...
		if *comments {
			fmt.Fprintf(w, "%s, // c0x%04x (%s)%s %s\n",
				formatThousands(c, 8), i, s, wildcardStr(wildcard), nodeTypeStr(nodeType))
		} else {
			fmt.Fprintf(w, "%s,\n", formatThousands(c, 0))
		}
	}
...
func printNode(w io.Writer, n *node) error {
	for _, c := range n.children {
		...
		if *comments {
			fmt.Fprintf(w, "%s, // n0x%04x c0x%04x (%s)%s %s %s %s\n",
				formatThousands(encoding, 8), c.nodesIndex, c.childrenIndex, s, wildcardStr(c.wildcard),
				nodeTypeStr(c.nodeType), icannStr(c.icann), c.label,
			)
		} else {
			fmt.Fprintf(w, "%s,\n", formatThousands(encoding, 0))
		}
	}
	return nil
}

func formatThousands(num uint32, padding int8) string {
	var formatter string
	if padding == 0 {
		formatter = "%x"
	} else {
		formatter = fmt.Sprintf("%%0*x", padding)
	}

	hex := fmt.Sprintf(formatter, num)
	hexLen := len(hex)
	if hexLen > 5 {
		if hexLen%4 != 0 {
			hex = fmt.Sprintf("%0*s", 4-(hexLen%4)+hexLen, hex)
		}

		for i := 4; i < len(hex); i += 5 {
			hex = hex[:i] + "_" + hex[i:]
		}
	}
