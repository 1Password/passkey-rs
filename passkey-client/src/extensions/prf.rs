...
use passkey_crypto::{CryptoBackend, hash::Sha256Backend};
use passkey_types::{
    Bytes,
    ctap2::{
    ...
    P: public_suffix::EffectiveTLDProvider + Sync + 'static,
{
    pub(super) fn registration_prf_to_ctap2_input(
        ...
        let maybe_prf = Self::make_ctap_extension(
    ...
    fn convert_eval_to_ctap(
        ...
        let (first, second) = if should_hash {
            let salt1 = Self::make_salt(&eval.first);
            let salt2 = eval.second.as_ref().map(Self::make_salt);
            (salt1, salt2)
        } else {
            let salt1 = eval
                .first
    ...
    fn make_ctap_extension(
        ...
        let prf = Self::validate_no_eval_by_cred(prf)?;
        ...
        let prf = prf
            ...
            .map(|prf| {
                ...
                prf.eval
                    .as_ref()
                    .map(|values| Self::convert_eval_to_ctap(values, should_hash))
                    .transpose()
    ...
    pub(super) fn auth_prf_to_ctap2_input(
        ...
        let maybe_prf = Self::get_ctap_extension(
    ...
    fn get_ctap_extension(
        ...
        let new_eval_by_cred = precomputed_eval_cred
            .map(|map| {
                map.into_iter()
                    .map(|(k, values)| {
                        Self::convert_eval_to_ctap(values, should_hash).map(|v| (k, v))
                    })
        ...
        let eval = prf_input
            .and_then(|prf| {
                prf.eval
                    .as_ref()
                    .map(|prf_values| Self::convert_eval_to_ctap(prf_values, should_hash))
            })
            .transpose()?;
